...
%%%% VERSION 3.8 12/13/10
%%%% Optimized Unix/Linux (Ubuntu)
...
for currentMovie = 1:numMovies
    ...
    for frameNum = skip:skip:(numFrames-1)
        ...
        if ((mod(frameNum,uint32(alignmentFreq*numFrames)) == 0) || strcmp('True',get(handles.reAlign,'String')))
            ...
            numWells = size(fishAreas,1);
            save(strcat(outputPath,'/',files(currentMovie,:),'.mat'),'fishDistances', 'fishCoords', 'fishQuants', 'noObjectError', 'fishAreas','frameRate','radius', 'tooManyObjectError','heatMap','unscaledRadius');
        end
        ...
        if (get(handles.displayChk,'Value') == 1)
            ...
            if (get(handles.wTrack,'Value'))
                ...
                axis off square;
                hold on;    
                % Plot the fish's centroid with a green *
                %plot(relativeFishLoc(watchWell,2), relativeFishLoc(watchWell,1), 'g*');
                % Plot the fish's centroid with a yellow dot
